---
title: "TD4_24-10"
author: "Lucile Dubarry"
format: 
  html:
    echo: false
    execute:  
      keep-md: true
---

```{r configuration}
#| message: false
here::i_am("r-101-grade-analysis.Rproj")
library(here)
library(tidyr)
library(dplyr)
library(vroom)
library(ggplot2)
library(knitr)
```

## Question 1 

```{r}
#| message: false
grades <- vroom(here("grades.csv"))
```

## Question 2

```{r}
variable_exam <- grades |>
  summarise(median = median(Exam, na.rm=TRUE), 
            mean = mean(Exam, na.rm=TRUE), 
            minimum = min(Exam, na.rm=TRUE), 
            maximum = max(Exam, na.rm=TRUE))
kable(variable_exam)
```

## Question 3

```{r}
NA_count <- grades |> filter(is.na(Exam))
```
`r count(NA_count)|>pull(n)` students were missing at the final exam. 

## Question 4

```{r}
students_exam <- grades |> filter(!is.na(Exam))
ggplot(students_exam, aes(x = Exam)) +
  geom_bar() +
  xlab("Grades at the exam") +
  ylab("Total of students")
```

## Question 5

```{r}
group_grades <- grades |> 
  group_by(Group)
group_count <- count(group_grades)
kable(group_count)
```

## Question 6

```{r}
ggplot(grades, aes(x=Group)) +
  geom_bar()
```

## Question 7

```{r}
step1 <- group_grades |> filter(!is.na(Exam))
ggplot(step1, aes(x = Exam, fill = Group)) +
  geom_bar() +
  xlab("Grades depending to groups") +
  ylab("Total of groups")
```

